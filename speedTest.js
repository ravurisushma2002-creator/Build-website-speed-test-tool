...
function pingWebsite(url) {
    ...
    req.on('error',(err)=>{
        const endTime=Date.now();
        const responseTime=endTime-startTime;
        console.log(`Filed to connect to ${hostname}: ${err.message}`);
        console.log(`Time elapsed before failure ${responseTime}ms`);
    });
    //Set a timeout for the request (3 seconds)
    req.setTimeout(3000, ()=>{
        req.abort();
        const responsibile = endTime-startTime;
        console.log(`Connection to ${hostname} time out`);
        console.log(`Time elapsed before timedout ${responseTime}ms`);
